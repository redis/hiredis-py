...
class ReaderTest(TestCase):
  ...
  def test_error_when_feeding_non_string(self):
    self.assertRaises(TypeError, self.feed(), 1)
  ...
  def test_subclassable(self):
    ...
    reader.feed_from(b"+ok\r\n")
    self.assertEquals(b"ok", reader.gets())

class ReaderTestRecvFrom(ReaderTest, TestCase):
  def feed(self):
    return self.reader.feed_from

  def test_recv_from(self):
    try:
      bytearray
    except NameError:
      pass
  ...
  def test_empty_buffer(self):
    try:
      ...
      data = bytearray()
      self.assertRaises(ValueError, self.feed(), data)
  ...
  def test_wrong_len(self):
    try:
      ...
      self.assertRaises(ValueError, self.feed(), data, 0, 5)
  ...
  def test_wrong_offset(self):
    try:
      ...
      data = bytearray(b"test")
      self.assertRaises(ValueError, self.feed(), data, 5, 5)
  ...
  def test_ok_offset(self):
    try:
      ...
      data = bytearray(b"blah*2\r\n$5\r\nhello\r\n$5\r\nworld\r\n")
      self.feed()(data, 4)
      self.assertEquals([b"hello", b"world"], self.reply())

  def test_ok_len(self):
    try:
      bytearray
    except NameError:
      pass
